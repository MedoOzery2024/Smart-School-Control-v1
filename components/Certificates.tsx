...
import { Award, Printer, LayoutTemplate, Palette, Type, Image as ImageIcon, PenTool } from 'lucide-react';
...
const Certificates: React.FC = () => {
  ...
  const [signatureTitle, setSignatureTitle] = useState('مدير المدرسة');

  const fonts = [
    { name: 'افتراضي (Cairo)', value: 'Cairo, sans-serif' },
    { name: 'رسمي (Serif)', value: '"Times New Roman", Times, serif' },
    { name: 'بسيط (Sans)', value: 'Arial, Helvetica, sans-serif' },
    { name: 'آلة كاتبة (Mono)', value: '"Courier New", Courier, monospace' },
  ];

  const renderCertificatePreview = () => {
    // Styles based on template
    const baseStyle = "w-full aspect-[1.414] bg-white text-black p-10 relative shadow-2xl overflow-hidden flex flex-col items-center text-center print:shadow-none transition-all duration-300";
    const commonStyle = { fontFamily: fontFamily };

    if (template === 'CLASSIC') {
      return (
        <div className={baseStyle} style={{ ...commonStyle, border: `20px double ${primaryColor}` }}>
           ...
           {showLogo && <div className="mb-4 scale-125"><Logo size={80} /></div>}
           <h1 className="text-4xl font-bold mt-2 mb-4">{MOCK_SCHOOL.name}</h1>
           ...
           <div className="mt-auto flex justify-between w-full px-20 mb-10">
              <div className="text-center">
                 <p className="border-t border-black pt-2 w-40">{signatureTitle}</p>
              </div>
              <div className="text-center">
    ...
    if (template === 'MODERN') {
       return (
        <div className={`${baseStyle} bg-slate-50`} style={commonStyle}>
           ...
           <div className="z-10 w-full">
              <div className="flex justify-between items-center w-full mb-12">
                 ...
                 {showLogo && <div className="p-2 bg-white rounded-full shadow-md"><Logo size={50} /></div>}
              </div>
              ...
              <div className="flex justify-center gap-20 mt-12">
                 <div className="text-center">
                    ...
                    <p className="text-sm text-gray-500">توقيع {signatureTitle}</p>
                 </div>
    ...
    // ELEGANT
    return (
      <div className={baseStyle} style={{ ...commonStyle, backgroundColor: '#fffbf0' }}>
         ...
         <div className="relative z-10 mt-10 w-full max-w-3xl">
            {showLogo && <div className="mx-auto mb-6 flex justify-center"><Logo size={64} /></div>}
            ...
            <div className="mt-16 flex justify-around w-full">
               <div className="text-center">
                  <p className="italic text-gray-500 mb-8">التاريخ</p>
                  <p className="font-bold">{new Date().toLocaleDateString('ar-EG')}</p>
               </div>
               <div className="text-center">
                  <p className="italic text-gray-500 mb-8">التوقيع</p>
                  <p className="font-bold text-xl">{signatureTitle}</p>
               </div>
            </div>
         </div>
      </div>
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-[calc(100vh-140px)]">
      {/* Settings Panel */}
      <div className="lg:col-span-1 bg-darkgray border border-gold-700/20 rounded-xl p-6 overflow-y-auto custom-scrollbar">
        ...
        <div className="space-y-6">
           <div>
              <label className="block text-sm text-gray-400 mb-2">القالب (Layout)</label>
                 ...
                 {['CLASSIC', 'MODERN', 'ELEGANT'].map(t => (
                   <button
                     ...
                     className={`p-2 text-xs rounded border transition-all ${
                       template === t ? 'bg-gold-600 text-black border-gold-600 font-bold' : 'bg-black/30 text-gray-400 border-gray-700 hover:border-gray-500'
                     }`}
                   >
           ...
           <div>
              <label className="block text-sm text-gray-400 mb-2 flex items-center gap-2">
                <Type className="w-4 h-4" /> نوع الخط (Font)
              </label>
              <select 
                value={fontFamily}
                onChange={(e) => setFontFamily(e.target.value)}
                className="w-full bg-black/40 border border-gray-700 rounded p-2 text-white text-sm focus:border-gold-500 outline-none"
              >
           ...
           <div>
              <label className="block text-sm text-gray-400 mb-2 flex items-center gap-2">
                <PenTool className="w-4 h-4" /> مسمى التوقيع
              </label>
              <input 
                type="text" 
           ...
           <div>
              <label className="block text-sm text-gray-400 mb-2 flex items-center gap-2">
                <ImageIcon className="w-4 h-4" /> خيارات الشعار
              </label>
              <label className="flex items-center gap-2 cursor-pointer bg-black/20 p-2 rounded border border-gray-800">
                 <input 
                   type="checkbox" 
                   checked={showLogo}
                 ...
                 />
                 <span className="text-sm text-gray-300">إظهار شعار المدرسة</span>
              </label>
           </div>
           ...
           <div>
              ...
              <div className="flex gap-3 flex-wrap">
                 {['#EAB308', '#3b82f6', '#ef4444', '#10b981', '#8b5cf6', '#ec4899', '#171717'].map(c => (
                    <button
                      key={c}
                      onClick={() => setPrimaryColor(c)}
                      className={`w-8 h-8 rounded-full border-2 transition-transform ${primaryColor === c ? 'border-white scale-110 shadow-lg shadow-white/20' : 'border-transparent hover:scale-105'}`}
           ...
           <button 
             ...
             className="w-full bg-gold-600 hover:bg-gold-500 text-black font-bold py-3 rounded-lg flex items-center justify-center gap-2 shadow-lg transition-colors"
           >
      ...
      <div className="lg:col-span-2 bg-gray-900/50 border border-gray-800 rounded-xl p-8 flex items-center justify-center overflow-auto relative">
         <div className="absolute top-4 right-4 bg-black/50 px-3 py-1 rounded text-xs text-gray-400 pointer-events-none">
            معاينة حية
         </div>
